...
router.post('/message', [
    ...
    try {
        ...
        logger_1.logger.info('Chatbot conversation', {
            sessionId,
            userMessage: message,
            aiResponse,
            userInfo,
        ...
        logger_1.logger.error('Chatbot message error:', error);
        res.status(500).json({
            success: false,
            error: 'Server error',
        });
...
router.post('/contact', [
    ...
    try {
        ...
        logger_1.logger.info('Chatbot contact created', {
            sessionId,
        ...
        logger_1.logger.error('Chatbot contact error:', error);
        res.status(500).json({
            success: false,
            error: 'Server error',
        });
...
router.get('/stats', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const chatbotContacts = yield contact_1.Contact.countDocuments({ source: 'chatbot' });
        ...
        const statusStats = yield contact_1.Contact.aggregate([
            { $match: { source: 'chatbot' } },
            {
                $group: {
                    _id: '$status',
        ...
        logger_1.logger.error('Get chatbot stats error:', error);
        res.status(500).json({
            success: false,
            error: 'Server error',
        });
